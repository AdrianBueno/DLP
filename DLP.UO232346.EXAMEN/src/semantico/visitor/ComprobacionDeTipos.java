...
public class ComprobacionDeTipos extends DefaultVisitor {
	...
	public Object visit(SenAsignacion node, Object param) {
		...
		if(predicado(node.getRights().size() == node.getLefts().size(),"Debe haber el mismo número de argumentos en ambas partes de una asignación",node)){
			for(Expresion expr : node.getLefts())
				expr.accept(this,null);
			for(Expresion expr : node.getRights())
				expr.accept(this,null);
			for(int i = 0; i < node.getLefts().size(); i++){
				if(!predicado(node.getLefts().get(i).isModificable(),"Error, la parte izquierda de la asignación no es modificable ",node)) //Left modificable
					break;
				if(!predicado(tipoPrimitivo(node.getLefts().get(i).getTipo()),"Error, la parte izquierda debe ser tipo Simple ",node)) //Left Simple
					break;
				if(!predicado(mismoTipo(node.getLefts().get(i).getTipo(), node.getRights().get(i).getTipo()),"Error, Las 2 partes deben tener mismo tipo ",node)) //Mismo tipo
					break;
			}
		}
		return ob;
