...
public class SeleccionDeInstrucciones extends DefaultVisitor {
	...
	public Object visit(SenAsignacion node, Object param) {
		...
		for(int i = 0; i < node.getLefts().size(); i++){
			node.getLefts().get(i).accept(direccionVisitor, null);
			node.getRights().get(i).accept(valorVisitor,null);
		}
		for(int i = node.getLefts().size()-1; i >= 0; i--)
			sh.code("STORE" + sh.getSubfijo(node.getRights().get(i).getTipo()));	
